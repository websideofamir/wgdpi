...
For high-volume downlink traffic, `--reply-mode mixed` wraps handshake/control replies but sends transport data replies plain. `--reply-mode adaptive` keeps transport replies wrapped but does not force fixed-size padding on transport replies by default. Use `--reply-transport-pad-to` with adaptive mode to set a smaller fixed transport reply target such as `1400`.
